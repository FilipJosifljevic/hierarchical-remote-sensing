...
def main():
    ...
    parser.add_argument("--checkpoint_every", type=int, default=5,
                        help="save a checkpoint every N epochs (protects against Colab/session disconnects -- "
                          "previously this script only saved once at the very end)")
    parser.add_argument("--resume_from", type=str, default=None,
                        help="path to a checkpoint to load model weights from before training. "
                          "NOTE: this only restores MODEL weights, not optimizer state or epoch count -- "
                          "training restarts its epoch counter and optimizer momentum from scratch. "
                          "Good enough to continue making progress after a disconnect, but epoch numbers "
                          "in this run won't line up with a from-scratch run of the same total length.")
    ...
    model = HELM(edge_index=edge_index, num_labels=num_labels, backbone_name=args.backbone_name, pretrained=True).to(args.device)
    if args.resume_from:
    ...
    for epoch in range(1, args.epochs + 1):
        model.train()
        epoch_losses = {"loss": 0.0, "L_s": 0.0, "L_g": 0.0, "L_b": 0.0}
        n_batches = 0
        ...
        print(f"Epoch {epoch}/{args.epochs} -- loss: {avg['loss']:.4f} "
              f"(L_s: {avg['L_s']:.4f}, L_g: {avg['L_g']:.4f}, L_b: {avg['L_b']:.4f}) "
              f"[{n_batches_with_labels}/{n_batches} batches had labeled samples]")

        if epoch % args.eval_every == 0 or epoch == args.epochs:
            interim_ckpt_path = os.path.join(
                args.checkpoint_dir,
                f"helm_frac{args.labeled_fraction}_seed{args.run_seed}_epoch{epoch}_interim.pt",
            )
            torch.save(model.state_dict(), interim_ckpt_path)
            print(f"  [checkpoint] saved interim checkpoint to {interim_ckpt_path}")
